...
public class OrderBookUsingSortedMapAndList implements OrderBookI {
    private final Map<Integer, Order> orderCache = new HashMap<>();
    private final NavigableMap<Double, PriceLevelList> buyPriceLevels = new TreeMap<>();
    private final NavigableMap<Double, PriceLevelList> sellPriceLevels = new TreeMap<>();
    ...
    private void addOrderAndMatch(final Order order) {
        ...
        if (order.getSide() == Side.BUY) {
            buyPriceLevels.computeIfAbsent(price, pl -> new PriceLevelList(price)).addOrder(order);
        } else {
            sellPriceLevels.computeIfAbsent(price, pl -> new PriceLevelList(price)).addOrder(order);
        }
    ...
    private void matchOrders() {
        while (!buyPriceLevels.isEmpty() && !sellPriceLevels.isEmpty() &&
            ...
            final Map.Entry<Double, PriceLevelList> buyEntry = buyPriceLevels.pollLastEntry(); // O(1)
            final Map.Entry<Double, PriceLevelList> sellEntry = sellPriceLevels.pollFirstEntry(); // O(1)

            final PriceLevelList buyPriceLevel = buyEntry.getValue();
            final PriceLevelList sellPriceLevel = sellEntry.getValue();
    ...
    private String getPriceLevelsString(final NavigableMap<Double, PriceLevelList> priceLevels) {
        final StringBuilder builder = new StringBuilder();
        for (final Double price : priceLevels.descendingKeySet()) {
            builder.append("\nPrice: ")
                   .append(price)
                   .append(", ");
            final PriceLevelList priceLevel = priceLevels.get(price);
    ...
    private void removeOrder(final NavigableMap<Double, PriceLevelList> priceLevels, final Order order) {
        final PriceLevelList priceLevel = priceLevels.get(order.getPrice());
